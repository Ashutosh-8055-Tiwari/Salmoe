...
import RateProductModal from '../../../../components/rate-product/RateProductModal';
import axios from 'axios';
import RateProductStar from "../../../../utils/stars.svg";
import { LuStar } from "react-icons/lu";
import UpdateRatingModal from '../../../../components/rate-product/UpdateRatingModal';
...
const OrderDetails = React.memo(() => {
    ...
    console.log("order data in order page",orderData);
    ...
    const fetchOrderDetails = async () => {
        api.getOrders(jwtToken, null, null, null, orderId).then(result => result.json()).then((response) => {
                ...
                <section className="order-details-page">
                    {/* <div className='cover'>
                        ...
                        <div className='page-heading'>
                            ...
                            <p><strong onClick={() => navigate.push('/')}>{t("home")}</strong> / <span> <span onClick={() => navigate.push('/profile/orders')}>{t("order")}</span> / {orderData?.id}</span></p>
                        </div>
                    </div> */}
                                                                    ...
                                                                    <tr className={Number(item?.active_status) > 6 ? 'disabled' : ''}>
                                                                        <td>
                                                                            <div className="product">
                                                                                ...
                                                                                <div className="image-container">
                                                                                    <img src={item.image_url} onError={placeHolderImage} alt="" />
                                                                                </div>
                                                                        ...
                                                                        <td>
                                                                            ...
                                                                            <div className="actions-container">
                                                                                ...
                                                                                {(Number(item?.active_status) <= 3) && (Number(item?.active_status) <= item?.till_status) && (item?.cancelable_status == 1) ?
                                                                                    <span className="cancel">
                                                                    ...
                                                                    {showCancelModal[index] ?
                                                                        ...
                                                                        >
                                                                            <Modal.Header className='d-flex justify-content-between returnProductModalHeader'>
                                                                                <h5 className='title text-black'>{t("cancel_order_item")}</h5>
