...
const FAQ = () => {
    ...
    return (
        <>
            <Head>
                <title>{t("faq")} - Kashmir Trout</title>
            </Head>
            {!isNetworkError ?
                <section id='faq' className='faq'>
                    <>
                        {/* <div className='cover'>
                            <img onError={placeHolderImage} src={coverImg.src} className='img-fluid' alt="cover"></img>
                            <div className='title'>
                                <h3>{t("faq")}</h3>
                                <span> <Link href="/" className='text-light text-deco text-decoration-none'>{t("home")} /</Link> </span><span className='active'>{t("faq")}</span>
                            </div>
                        </div> */}
                                                    ...
                                                    {faqs.data.map((faq, index) => (
                                                        <div key={index} className='faq-card'>
                                                            <button type='button' data-bs-toggle="collapse" data-bs-target={"#collapseExample" + index} aria-expanded="false" aria-controls="collapseExample" onClick={() => {
                                                                ...
                                                                <span id={`faq-icon1${index}`} className='icon active'><AiOutlinePlus fontSize={"2rem"} fill="#fff" /></span>
                                                                <span id={`faq-icon2${index}`} className='icon'><AiOutlineMinus fontSize={"2rem"} fill="#fff" /></span>
                                                            </button>
