...
const ViewCart = () => {
    ...
    useEffect(() => {
        if (navigate.pathname == "/cart" && cart?.isGuest === false) {
        ...
        } else if (navigate.pathname == "/cart" && cart?.isGuest === true && cart?.guestCart?.length !== 0) {
            fetchGuestCart();
        } else if (navigate.pathname == "/cart" && cart?.isGuest === true && cart?.guestCart?.length === 0) {
            setiscartEmpty(true);
        }
    ...
    useEffect(() => {
        if (navigate.pathname == "/cart" && cart?.isGuest === true) {
            fetchGuestCart();
        }
    }, []);
            ...
            {!isNetworkError ?
                <section id='viewcart' className='viewcart'>
                    {/* <div className='cover'>
                        ...
                        <div className='title'>
                            <h3>{t("cart")}</h3>
                            <span><Link href='/' className='text-light text-decoration-none'>{t("home")} / </Link></span><span className='active'>{t('cart')}</span>
                        </div>
