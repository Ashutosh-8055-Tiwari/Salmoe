...
const ProductCards = ({ product , isCartSidebarOpen, setIsCartSidebarOpen}) => {
    ...
    console.log("in product container ", product.image_url)


    return (
        <div >
            <div className="product-grid new-product-cardss border border-2 p-3 rounded-5" >
                <div >
                    <div className="new-product-image">
                        <a onClick={() => navigate.push(`/products/${product.slug}`)} className="image">
                            <ImageWithPlaceholder src={product?.image_url} alt={product?.slug} />
                            {product?.indicator ?
                                <div className='product-vegen-type'>
                                    {product?.indicator == 1 ? <img src={VegIcon.src} alt="Veg icons" /> : <img src={NonVegIcon.src} alt="non Veg icons" />}
                    ...
                    <div className='d-flex flex-column justify-content-center align-items-center'>
                         <div onClick={() => navigate.push(`/products/${product.slug}`)}><h3 className="title fs-5"><a > {product?.name} </a></h3></div>
                         
                           <div className='product-name-price mb-1 '>
                           
                           <div><button className='product-qty-btn' onClick={() => handleVariantModal(product)}>
                    ...
                    product?.variants?.length <= 1 && product?.variants?.[0]?.is_unlimited_stock == 0 && product?.variants?.[0]?.stock == 0 ? <span className='variant-out-of-stock'>{t("OutOfStock")} </span> :
                        <div className='product-btn'>
                            ...
                            {cart?.isGuest === false && cart?.cartProducts?.find((prdct) => prdct?.product_variant_id == selectedVariant?.id)?.qty > 0 ||
                                ...
                                : <button className='product-cart-btn1'   onClick={() => {
                                    if (cart?.isGuest) {
                                        const quantity = getProductQuantities(cart?.cartProducts)
