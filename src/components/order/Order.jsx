...
const Order = () => {
    ...
    const getInvoice = async (Oid) => {
        ...
        axios({
            url: `${process.env.NEXT_PUBLIC_API_URL}${process.env.NEXT_PUBLIC_API_SUBURL}/invoice_download`,
            method: 'post',
            responseType: 'blob',
    ...
    const getImageofOrderStatus = (status) => {
        return (
            <>
                {
                    status == 1 ? <img src={PendingPaymentSVG.src} className='p-3' alt='PendingPaymentSVG' /> :
                        status == 2 ? <img src={ReceivedSVG.src} className='p-3' alt='ReceivedSVG' /> :
                            status == 3 ? <img src={ProcessedSVG.src} className='p-3' alt='ProcessedSVG' /> :
                                status == 4 ? <img src={ShippedSVG.src} className='p-3' alt='ShippedSVG' /> :
                                    status == 5 ? <img src={OutforDeliverySVG.src} className='p-3' alt='OutforDeliverySVG' /> :
                                        status == 6 ? <img src={DeliveredSVG.src} className='p-3' alt='DeliveredSVG' /> :
                                            status == 7 ? <img src={CancelledSVG.src} className='p-3' alt='CancelledSVG' /> :
                                                status == 8 ? <img src={ReturnedSVG.src} className='p-3' alt='ReturnedSVG' /> : null
                }
            </>
        );
    };
                                    ...
                                    <>
                                        {ActiveOrders && ActiveOrders.length === 0
                                            ? <div className='d-flex align-items-center p-4 no-orders'>
                                                <img src={No_Orders.src} className='no-data-img' alt='no-orders'></img>
                                                <p>{t("no_order")}</p>
                                            </div>
                                            :
                                                    ...
                                                    {ActiveOrders && ActiveOrders.map((order, index) => (
                                                        <tr key={order.order_id} className={index === ActiveOrders.length - 1 ? 'last-column' : ''}>
                                                            ...
                                                            <th className='button-container'>
                                                                ...
                                                                <button onClick={() => {
                                                                    navigate.push(`orders/${order.order_id}`);
                                ...
                                >
                                    <>
                                        {PrevOrders && PrevOrders.length === 0
                                            ? <div className='d-flex align-items-center p-4 no-orders'>
                                                <img src={No_Orders.src} alt='no-orders'></img>
                                                <p>{t("no_order")}</p>
                                            </div>
                                            :
                                            <table className='order-list-table'>
                        ...
                        <div className="modal fade new-track" id="trackModal" aria-labelledby="TrackModalLabel" aria-hidden="true">
                            <div className='modal-dialog'>
                                <div className="modal-content" style={{ borderRadius: "10px", maxWidth: "100%", padding: "30px 15px", zIndex: 2 }}>
                                    <div id="mainContentTrack" className='z-20'>
                                        <section className="track z-20" id="printMe">
                                            <div className="d-flex justify-content-between align-items-center mx-5">
                                                ...
                                                <button type="button" className="closeBtn" data-bs-dismiss="modal" aria-label="Close" ref={closeModalRef} style={{ width: '30px' , zIndex:100}}><AiOutlineCloseCircle size={26} /></button>
                                            </div>
