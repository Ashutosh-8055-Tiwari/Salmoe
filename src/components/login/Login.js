...
const Login = React.memo((props) => {
    ...
    const countryDialCode = process.env.NEXT_PUBLIC_COUNTRY_DIAL_CODE
    const phoneNumber = process.env.NEXT_PUBLIC_DEMO_LOGIN_NO
                    ...
                    {isOTP
                        ? (
                            <form className='d-flex flex-column gap-3 form w-100' onSubmit={verifyOTP}>
                                ...
                                <OTPInput className='otp-container' inputStyle="otp-container-style" value={OTP} onChange={setOTP} autoFocus OTPLength={6} otpType="number" disabled={false}  autoComplete="one-time-code"/>
                                <span className='timer' >
                                    <button onClick={handleLogin} disabled={disabled} className='border-0'>
                                        {timer === 0 ?
                        ...
                        : (
                            <div>
                                ...
                                {setting?.setting?.phone_login == 1 ? <form className='d-flex flex-column gap-3 form' onSubmit={handleLogin}>
                                    ...
                                    <div>
                                    <PhoneInput
    country="IN" // Fixes the country code to +91 (India)
    value="+91"  // Ensures the input starts with +91
    onChange={handleOnPhoneChange}
    enableSearch
    disableSearchIcon
    placeholder={t('please_enter_valid_phone_number')}
    disableDropdown={true} // Prevents users from changing the country code
...
/>
                                    ...
                                    {/* <span style={{ alignSelf: "baseline" }}>
                                        ...
                                        {t("agreement_message")} &nbsp;<Link href={"/terms"} onClick={handleTerms}>{t("terms_of_service")}</Link> &nbsp;{t("and")}
                                        <Link href={"/policy/Privacy_Policy"} onClick={handlePolicy}>&nbsp; {t("privacy_policy")} &nbsp;</Link>
                                    </span> */}
                                ...
                                {setting?.setting?.google_login == 1 ? <div className='google-auth-container'>
                                    <button className='login-google-btn' onClick={handleGoogleAuthentication}><img src={GoogleLogo.src} className='google-log-img' />{t("continue_with_google")}</button>
                                </div> : <></>}
                                ...
                                <span style={{ alignSelf: "baseline", marginTop: "20px", fontSize: "12px" }}>
                                    ...
                                    {t("agreement_updated_message")} &nbsp;<Link href={"/terms"} onClick={handleTerms}>{t("terms_of_service")}</Link> &nbsp;{t("and")}
                                    <Link href={"/policy/Privacy_Policy"} onClick={handlePolicy}>&nbsp; {t("privacy_policy")} &nbsp;</Link>
                                </span>
                            </div>
                        )}
